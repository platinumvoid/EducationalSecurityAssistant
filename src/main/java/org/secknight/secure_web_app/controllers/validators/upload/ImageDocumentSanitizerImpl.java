...
public class ImageDocumentSanitizerImpl implements DocumentSanitizer {
    ...
    public boolean madeSafe(File f) {
        ...
        try {
            if ((f != null) && f.exists() && f.canRead() && f.canWrite()) {
                ...
                try (OutputStream fos = Files.newOutputStream(f.toPath(), StandardOpenOption.WRITE)) {
                    if (!fallbackOnApacheCommonsImaging) {
                        ...
                        switch (formatName) {
                            case "TIFF" -> imageParser = new TiffImageParser();
                            case "PCX" -> imageParser = new PcxImageParser();
                            case "DCX" -> imageParser = new DcxImageParser();
                            case "BMP" -> imageParser = new BmpImageParser();
                            case "GIF" -> imageParser = new GifImageParser();
                            case "PNG" -> imageParser = new PngImageParser();
                            case "WBMP" -> imageParser = new WbmpImageParser();
                            case "XBM" -> imageParser = new XbmImageParser();
                            case "XPM" -> imageParser = new XpmImageParser();
                            default -> throw new IOException("Format of the original image is not supported for write operation !");
                        }
            ...
            safeState = false;
            LOG.warn("Error during Image file processing !");
        }
